...
> 2026-05-26 用户覆盖说明：本文件中关于 deterministic render、HTML/PNG/PDF 脚本渲染、A1/A2 渲染验收的内容已暂停作为执行路线。当前只接受“底图 -> 用户人工草图 -> 保存 semantic JSON”的工作台能力；不得让 agent 或脚本自动生成技术图设计结论。后续如需成品图，必须重新提出方案并经用户确认。
