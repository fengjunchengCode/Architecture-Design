...
图片资料（`visual_asset`，如 JPG/PNG 区位图、现场照片）必须通过 `python _tools/vision_route.py {项目代号} --write` 自动路由到 `VISION_MODEL`。不得要求用户手动切换 API 模型；若 `OPENAI_API_KEY` 或 `VISION_MODEL` 未配置，则读取 `05_output/vision/` 降级 sidecar，并把地址、坐标、红线等缺口写入 `pending_questions` 或 `low_confidence_fields`。
